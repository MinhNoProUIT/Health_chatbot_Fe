...
async function seed() {
    ...
    for (const item of items) {
        try {
            ...
            console.log(`✅ Inserted bill: userId=${item.userId}, visitId=${item.visitId}, hospital=${item.hospitalName}`);
        }
        catch (error) {
            console.error(`❌ Failed to insert userId=${item.userId}, visitId=${item.visitId}:`, error.message);
        }
    }
    console.log("\n🎉 Seeding completed!");
